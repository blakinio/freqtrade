...
- After cleanup, verify both that the intended task-owned resources are gone and that protected/current services remain healthy using their declared Docker health checks and/or service-level probes where available; process `running` state alone is not sufficient when a stronger health signal exists. Record exact resource names/IDs and the health evidence in the task closeout.
