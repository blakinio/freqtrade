...
class SourceDeclaration:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.market_types or len(set(self.market_types)) != len(
            self.market_types
        ):
...
class SourceCatalog:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or self.catalog_version != "market-data-source-catalog-v1"
        ):
        ...
        if len(self.sources) != 6 or len(set(source_ids)) != len(source_ids):
            raise ValueError(
                "catalog must contain exactly six unique initial source declarations"
            )
        expected_sources = {
            "binance-spot",
            "binance-usdm",
        ...
        if set(source_ids) != expected_sources:
            raise ValueError(
                "catalog source set does not match the bounded initial venue set"
            )
        required_boundaries = {
        ...
        try:
            sources = tuple(
                SourceDeclaration(
                    ...
                    market_types=tuple(
                        MarketType(str(value)) for value in item["market_types"]
                    ),
