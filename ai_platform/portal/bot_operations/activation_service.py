...
from ai_platform.portal.execution_submission.errors import ExecutionSubmissionError
...
class RuntimeDryRunVerifier(Protocol):
    def verify_dry_run(
        self,
...
class ReplacementSubmissionService(Protocol):
...
class BotCommandActivationService:
    def __init__(
        ...
        dry_run_verifier: RuntimeDryRunVerifier,
        command_transport: PrivateRuntimeCommandTransport,
        replacement_service: ReplacementSubmissionService,
    ) -> None:
    ...
    def activate_position(
        ...
        preflight = _preflight_result(outcome)
        ...
        try:
            ...
            return _failed_activation(
                pending,
                attempt_ref,
                CommandActivationState.AMBIGUOUS,
                exc.reason_code,
            )
            ...
            return _failed_activation(
                pending,
                attempt_ref,
                CommandActivationState.REJECTED,
                exc.reason_code,
            )
        except (CredentialBrokerError, ExecutionSubmissionError) as exc:
            return _failed_activation(
                pending,
                attempt_ref,
                CommandActivationState.REJECTED,
                getattr(exc, "reason_code", "COMMAND_ACTIVATION_DEPENDENCY_UNAVAILABLE"),
            )
        ...
        return _acknowledged_activation(pending, attempt_ref, acknowledgement)
    ...
    def _execute_position(
        ...
        with self._broker.resolve(lease_request) as lease:
            ...
            if request.command.action == PositionAction.CLOSE_ALL:
    ...
    def _execute_order(
        ...
        with self._broker.resolve(lease_request) as lease:
            self._dry_run_verifier.verify_dry_run(target, lease)
            if request.command.action == OrderAction.CANCEL_ALL_ORDERS:
                acknowledgements = [
                    self._transport.cancel_open_order(target, lease, trade_id=trade_id)
        ...
        try:
            ...
            raise CommandActivationAmbiguousError() from exc
        ...
        if acknowledgement is None or acknowledgement.status != AcknowledgementStatus.ACCEPTED:
            raise CommandActivationRejectedError()
...
def _preflight_result(outcome: CommandOutcome) -> CommandActivationResult | None:
...
def _acknowledged_activation(
    outcome: CommandOutcome,
    attempt_ref: str,
    acknowledgement: RuntimeCommandAcknowledgement,
) -> CommandActivationResult:
    return CommandActivationResult(
        outcome=outcome,
