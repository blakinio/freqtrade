...
CREATE TABLE IF NOT EXISTS portal_bot_commands (
...
CREATE INDEX IF NOT EXISTS ix_portal_bot_commands_tenant_created
    ON portal_bot_commands (scope_tenant_id, created_at);

CREATE TABLE IF NOT EXISTS portal_bot_command_history (
...
CREATE INDEX IF NOT EXISTS ix_portal_bot_command_history_tenant_command
    ON portal_bot_command_history (scope_tenant_id, command_id, sequence);

CREATE TABLE IF NOT EXISTS portal_bot_command_idempotency_conflicts (
    ...
    recorded_at TIMESTAMPTZ NOT NULL,
    CONSTRAINT fk_portal_bot_command_conflict_existing_command
        FOREIGN KEY (scope_tenant_id, existing_command_id)
        REFERENCES portal_bot_commands (scope_tenant_id, command_id)
        ON DELETE RESTRICT
);

CREATE INDEX IF NOT EXISTS ix_portal_bot_command_conflicts_tenant_key
