...
class ReconciliationRecord(ContractModel):
    ...
    def validate_reconciliation(self) -> Self:
        ...
        evidence_keys = [
            (item.evidence_type.value, item.evidence_id) for item in self.evidence_refs
        ]
        ...
        for evidence in self.evidence_refs:
            if evidence.tenant_id != self.binding.tenant_id:
                raise ValueError("reconciliation evidence tenant mismatch")
            if evidence.bot_id != self.binding.bot_id:
                raise ValueError("reconciliation evidence bot mismatch")
            if evidence.config_revision != self.binding.config_revision:
                raise ValueError("reconciliation evidence config revision mismatch")
            if evidence.runtime_id != self.binding.runtime_id:
                raise ValueError("reconciliation evidence runtime mismatch")
            if evidence.runtime_revision != self.binding.runtime_revision:
        ...
        if terminal != (self.reconciled_at is not None):
            raise ValueError("terminal reconciliation state and reconciled_at must agree")
        if self.reconciled_at is not None and self.reconciled_at < self.started_at:
            raise ValueError("reconciled_at must not be before started_at")
        if self.state == ReconciliationState.SUCCEEDED:
            ...
            if not any(evidence.authoritative for evidence in self.evidence_refs):
                raise ValueError("successful reconciliation requires authoritative evidence")
        if self.state in {ReconciliationState.FAILED, ReconciliationState.CONFLICT}:
