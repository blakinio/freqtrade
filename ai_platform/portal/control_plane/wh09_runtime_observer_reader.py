...
def _bounded_decision_paths(decisions_root: Path) -> list[Path] | None:
    paths: list[Path] = []
    overflow = False
    try:
...
def _validated_decision(
    ...
    return observed_at_ms, {**payload, "record_sha256": record_hash}


def _latest_decision_model(payload: dict[str, Any]) -> Wh09LatestDecision:
    try:
...
class Wh09ObserverRuntimeEvidenceReader(Wh09RuntimeEvidenceReader):
    ...
    def _latest_decision(
        ...
        paths = _bounded_decision_paths(decisions_root)
        if paths is None:
            return None

        latest: tuple[int, dict[str, Any]] | None = None
        for path in paths:
            candidate = _validated_decision(path, identity)
            if latest is None or candidate[0] > latest[0]:
                latest = candidate

        if latest is None:
            return None
        return _latest_decision_model(latest[1])
