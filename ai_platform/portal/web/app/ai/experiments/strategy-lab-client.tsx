...
export function StrategyLabClient({ strategies, initialExperiments, initialBundle, initialComparison }: Props) {
  ...
  return (
    <section className="page-stack" data-testid="strategy-lab">
      ...
      <article className="panel">
        <h2>Eksperymenty</h2>
        <div className="table-wrap"><table><thead><tr><th>Status</th><th>Strategia</th><th>Para / TF</th><th>Data</th><th>Transakcje</th><th>Wynik</th><th>Drawdown</th></tr></thead><tbody>
          {experiments.map((item) => <tr key={item.experiment_id} onClick={() => openExperiment(item.experiment_id)} style={{ cursor: "pointer" }}><td><StatusPill value={item.status} /></td><td><strong>{item.strategy_id}</strong><span>{item.strategy_version}</span></td><td>{item.pair}<span>{item.timeframe}</span></td><td>{new Date(item.started_at).toLocaleString("pl-PL")}</td><td>{item.trade_count}</td><td>{money(item.profit_abs)} ({percent(item.profit_pct)})</td><td>{percent(item.max_drawdown)}</td></tr>)}
        </tbody></table></div>
      </article>
