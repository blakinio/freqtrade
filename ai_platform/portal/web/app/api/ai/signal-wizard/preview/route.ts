import { NextRequest, NextResponse } from "next/server";

import {
  identityErrorResponse,
  requireBrowserMutation,
  requireBrowserSession,
} from "@/lib/identity";
import {
  getSignalWizardBootstrap,
  previewSignalWizard,
  SignalWizardApiError,
} from "@/lib/signal-wizard-api";
import {
  isSignalWizardPreviewRequest,
  type SignalWizardFixtureView,
} from "@/lib/signal-wizard-contracts";

const fixtureViews = new Set<SignalWizardFixtureView>([
  "default",
  "empty",
  "stale",
  "failure",
  "leakage",
  "conflict",
]);

export async function GET(request: NextRequest) {
  try {
    requireBrowserSession(request);
    const payload = await getSignalWizardBootstrap(request, fixtureView(request));
    return NextResponse.json(payload, { headers: noStoreHeaders() });
  } catch (error) {
    return errorResponse(error, "Signal Wizard registry request failed closed");
  }
}

export async function POST(request: NextRequest) {
  try {
    requireBrowserMutation(request);
    const payload: unknown = await request.json();
    if (!isSignalWizardPreviewRequest(payload)) {
      return NextResponse.json(
        {
          detail:
            "Preview requires a strategy identity, current registry snapshot, approved feature selections, constraints, typed condition AST and idempotency key",
          reason_code: "SIGNAL_WIZARD_PREVIEW_REQUEST_INVALID",
        },
        { status: 422, headers: noStoreHeaders() },
      );
    }
    const result = await previewSignalWizard(request, payload, fixtureView(request));
    return NextResponse.json(result, { status: 200, headers: noStoreHeaders() });
  } catch (error) {
    return errorResponse(error, "Signal Wizard preview failed closed");
  }
}

function fixtureView(request: NextRequest): SignalWizardFixtureView {
  const value = request.nextUrl.searchParams.get("view") ?? "default";
  return fixtureViews.has(value as SignalWizardFixtureView)
    ? (value as SignalWizardFixtureView)
    : "default";
}

function errorResponse(error: unknown, fallback: string): NextResponse {
  const identityResponse = identityErrorResponse(error);
  if (identityResponse) return identityResponse;
  if (error instanceof SignalWizardApiError) {
    return NextResponse.json(
      {
        detail: error.message,
        ...(error.reasonCode ? { reason_code: error.reasonCode } : {}),
      },
      { status: error.status, headers: noStoreHeaders() },
    );
  }
  return NextResponse.json(
    { detail: fallback, reason_code: "SIGNAL_WIZARD_REQUEST_FAILED" },
    { status: 502, headers: noStoreHeaders() },
  );
}

function noStoreHeaders(): HeadersInit {
  return { "cache-control": "no-store" };
}
