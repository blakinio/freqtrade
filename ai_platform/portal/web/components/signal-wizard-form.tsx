...
export function SignalWizardForm({ bots }: { bots: BotInstance[] }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage(`Signal evidence recorded (${payload.signal_id ?? "new signal"}). No execution was triggered.`);
  ...
  return (
    <form className="bot-form" onSubmit={submit}>
      <div className="form-safety">
        ...
        <span>Recording a signal does not create a TradeIntent, bypass risk controls, or submit an order.</span>
      </div>
      <div className="form-grid">
        <label>
          ...
          <select
            ...
            required
          >
            {bots.map((bot) => <option key={bot.bot_id} value={bot.bot_id}>{bot.name} · {bot.bot_id}</option>)}
          </select>
        </label>
        <label>
          Pair
          <select name="pair" key={selectedBotId} defaultValue={selectedBot?.spec.pair_universe[0]} required>
        ...
        <label>Timeframe<input name="timeframe" value={selectedBot?.spec.timeframe ?? ""} readOnly /></label>
        <label>Confidence<input name="confidence" type="number" min="0" max="1" step="0.01" defaultValue="0.75" required /></label>
      </div>
      <label>Rationale<textarea name="rationale" defaultValue="Manual advisory signal recorded for review." required /></label>
