...
test.describe("Portal authenticated response cache boundary", {
  ...
  test("development documents, redirects and API outcomes cannot become shared-cacheable", async ({
    ...
    const extensionLookingProtectedPath = await page.request.get("/protected-workspace.png", {
      maxRedirects: 0,
    });
    expect([307, 308]).toContain(extensionLookingProtectedPath.status());
    assertNotSharedCache(extensionLookingProtectedPath.headers());
    ...
    const validationError = await page.request.post("/api/bots", {
      headers: {
    ...
    assertNotSharedCache(validationError.headers());
  });
