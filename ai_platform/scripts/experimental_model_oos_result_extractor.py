...
from ai_platform.scripts.run_experiment import ExperimentError, load_manifest, validate_research_config
...
def _load_contract(path: Path = DEFAULT_CONTRACT) -> dict[str, Any]:  # noqa: C901
    ...
    if contract.get("contract_id") != "experimental-model-research-strict-oos-extraction-v1":
        raise ExperimentalModelOosExtractorError("Unexpected experimental OOS extraction contract_id")
    ...
    if contract.get("schema") != "ai_platform/experimental_model_research/oos-extraction-schema-v1.json":
    ...
    if set(contract.get("allowed_tracks", [])) != EXPECTED_TRACKS:
        raise ExperimentalModelOosExtractorError("Experimental OOS allowed track identities drifted")
    ...
    if inclusion.get("open_date_operator") != ">=" or inclusion.get("close_date_operator") != "<":
        raise ExperimentalModelOosExtractorError("Experimental OOS timestamp boundary semantics drifted")
    ...
    if inclusion.get("missing_or_invalid_timestamp") != "fail_closed":
        raise ExperimentalModelOosExtractorError("Invalid timestamp handling must remain fail-closed")
...
def _load_track_and_manifest(
    ...
    canonical_manifest_path = _resolve_repo_path(track["manifest"], f"{track_id} canonical manifest")
...
def _write_json(path: Path, payload: dict[str, Any]) -> None:
    try:
        ...
        raise ExperimentalModelOosExtractorError(f"Unable to write extraction output: {exc}") from exc
