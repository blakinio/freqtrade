#!/usr/bin/env python3
"""Generate and validate the exact one-shot residual PyTorch M1 v2 request."""
...
EXPECTED_REQUEST_ID = "residual-pytorch-bounded-m1-execution-v2"
EXPECTED_ACTION = "execute_residual_pytorch_bounded_m1_v2_development_comparison"


class ResidualPyTorchBoundedM1V2RunRequestError(RuntimeError):
    pass


def _execution_module() -> ModuleType:
    try:
        return import_module("ai_platform.scripts.residual_pytorch_bounded_m1_v2_execution")
    except ModuleNotFoundError as exc:
        if exc.name in {"numpy", "pandas"}:
            raise ResidualPyTorchBoundedM1V2RunRequestError(
...
def canonical_run_request() -> dict[str, Any]:
    ...
    try:
        ...
        raise ResidualPyTorchBoundedM1V2RunRequestError(str(exc)) from exc
...
def load_run_request(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ResidualPyTorchBoundedM1V2RunRequestError(
            f"Unable to read bounded M1 v2 run request {path}: {exc}"
        ) from exc
    if not isinstance(request, dict):
        raise ResidualPyTorchBoundedM1V2RunRequestError("Run request must contain a JSON object")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.print_canonical:
        ...
        except ResidualPyTorchBoundedM1V2RunRequestError as exc:
    ...
    try:
        ...
        print(f"Bounded M1 v2 run request invalid: {exc}", file=sys.stderr)
        return 1
    if args.request.as_posix() != REQUEST_REPO_PATH:
        print("Bounded M1 v2 request path is not canonical", file=sys.stderr)
        return 1
