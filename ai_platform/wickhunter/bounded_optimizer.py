    ...
    def __post_init__(self) -> None:
        ...
        for group in split_groups:
            if not group or group != tuple(sorted(set(group))):
                raise BoundedOptimizerError(
                    "split groups must be non-empty, unique and sorted"
                )
            if any(not item.strip() for item in group):
                raise BoundedOptimizerError("split names must be non-empty")
        research_splits = (
            set(self.training_splits)
            | set(self.validation_splits)
            | set(self.test_splits)
        )
        if len(research_splits) != (
            len(self.training_splits)
            + len(self.validation_splits)
            + len(self.test_splits)
        ):
            raise BoundedOptimizerError(
                "training, validation and test splits must be disjoint"
            )
    ...
    def __post_init__(self) -> None:
        ...
        if self.selection_kind is TrialSelectionKind.INITIAL:
            if any(value is not None for value in surrogate_values):
                raise BoundedOptimizerError(
                    "initial trial cannot contain surrogate evidence"
                )
...
class BoundedOptimizationResult:
    ...
    def __post_init__(self) -> None:
        ...
        if self.top_parameter_sha256s != tuple(
            item.parameter_sha256 for item in self.stability
        ):
...
def _objective(
    ...
    *,
    ...
    selected_ratio = Decimal(report.overall.selected_count) / Decimal(
        report.overall.decision_count
    )
    net_return_mean = report.overall.net_return_mean or Decimal("0")
    slice_means = [
        item.net_return_mean
        for item in report.slices
        if item.dimension in {"symbol", "regime", "side"}
        and item.net_return_mean is not None
    ]
    if len(slice_means) > 1:
        ...
        variance = sum((value - center) ** 2 for value in numeric) / Decimal(
            len(numeric)
        )
...
def _normalized_vectors(
    ...
    return {
        candidate.parameter_hash: normalized[index]
...
def optimize_bounded_parameters(
    *,
    ...
    candidates: Sequence[WickHunterParameters],
    parameter_bounds: WickHunterParameterBounds,
    policy: BoundedOptimizerPolicy = BoundedOptimizerPolicy(),
    ...
    initial_indices = tuple(
        sorted(
            rng.choice(
                len(ordered_candidates), size=policy.initial_trials, replace=False
            ).tolist()
        )
    )
    ...
    evidence_by_hash = {
        trial.parameter_sha256: trial.validation_evidence for trial in trials
    }
    stability = tuple(
        StabilityEvidence(
            rank=rank,
            ...
            validation_slice_dispersion=evidence_by_hash[
                parameter_hash
            ].slice_dispersion,
