...
class MarketEvidenceV2PublicationError(RuntimeError):
    """Raised when two verified immutable source packages cannot be merged safely."""
            ...
            with path.open("rb") as source:
                for line in source:
                    ...
                    try:
                        ...
                        raise MarketEvidenceV2PublicationError("NDJSON source contains invalid JSON") from exc
...
def _capture_geometry(manifest: Mapping[str, object]) -> dict[str, int]:
    ...
    return {
        "pre_roll_start_ms": _integer(capture.get("pre_roll_start_ms"), field="pre-roll start"),
        "decision_start_ms": _integer(capture.get("decision_start_ms"), field="decision start"),
        "decision_end_ms": _integer(capture.get("decision_end_ms"), field="decision end"),
    }
...
def _copy_candles(
    *,
    ...
    for artifact in artifacts:
        ...
        if source not in expected_sources or symbol not in v1_core.EXPECTED_SYMBOLS:
        ...
        _copy_verified(source_path, destination, expected_sha256=expected_sha256)
        copied_identity = _identity(destination, root=destination_root)
        copied.append(
            {
                **dict(artifact),
                "normalized_file": copied_identity,
            }
        )
    expected = {
...
def merge_verified_packages(
    *,
    ...
    base_manifest = _load_json(base_package / PACKAGE_MANIFEST_NAME, field="base manifest")
    supplement_manifest = _load_json(supplement / "manifest.json", field="supplement manifest")
    if base_manifest.get("run_id") != supplement_manifest.get("base_v1_run_id"):
        raise MarketEvidenceV2PublicationError("base run identity does not match supplement binding")
    ...
    if any(base_manifest.get("authorities", {}).get(key) != value for key, value in v2.AUTHORITY.items()):
        raise MarketEvidenceV2PublicationError("base package authority boundary mismatch")
    ...
    partial_root.mkdir()
    try:
        base_request = _load_json(base_package / PACKAGE_REQUEST_NAME, field="base request")
        supplement_request = _load_json(supplement / "request.json", field="supplement request")
        ...
        base_index = json.loads((base_package / PACKAGE_CANDLE_INDEX_NAME).read_text(encoding="utf-8"))
        ...
        manifest: dict[str, object] = {
            ...
            "wh01": {
                ...
                "blocker_detail": (
                    "A compatible accepted immutable Liquid20 archive and prospective "
                    "WH-01 split binding are still required."
                ),
            },
            "artifacts": artifacts,
        ...
        checksum_identities = [
            *artifacts,
            _identity(partial_root / PACKAGE_MANIFEST_NAME, root=partial_root),
        ]
        _write_new(
...
def verify_combined_package(package_root: Path) -> dict[str, object]:  # noqa: C901
    if package_root.is_symlink() or not package_root.is_dir():
        raise MarketEvidenceV2PublicationError("combined package root must be a regular directory")
    manifest = _load_json(package_root / PACKAGE_MANIFEST_NAME, field="combined manifest")
    ...
    if manifest.get("sources") != list(v2.EXPECTED_SOURCES):
        raise MarketEvidenceV2PublicationError("combined source coverage mismatch")
    if manifest.get("instruments") != list(v1_core.EXPECTED_SYMBOLS):
        raise MarketEvidenceV2PublicationError("combined instrument coverage mismatch")
    if manifest.get("record_counts") != EXPECTED_COUNTS:
        raise MarketEvidenceV2PublicationError("combined record counts mismatch")
    if manifest.get("authorities") != v2.AUTHORITY:
        raise MarketEvidenceV2PublicationError("combined authority boundary mismatch")
    binding = _load_json(package_root / PACKAGE_BINDING_NAME, field="source binding")
    binding_claim = binding.get("binding_sha256")
    binding_seed = dict(binding)
    binding_seed.pop("binding_sha256", None)
    if not isinstance(binding_claim, str) or _canonical_hash(binding_seed) != binding_claim:
        raise MarketEvidenceV2PublicationError("source binding self hash mismatch")
    if manifest.get("source_package_binding_sha256") != binding_claim:
        raise MarketEvidenceV2PublicationError("manifest binding identity mismatch")
    ...
    verification = _load_json(package_root / PACKAGE_VERIFICATION_NAME, field="verification")
    ...
    if any(verification.get(key) != value for key, value in v2.AUTHORITY.items()):
        raise MarketEvidenceV2PublicationError("verification authority boundary mismatch")
    return {
        "status": "published",
        "outcome": "accepted",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Merge and verify WickHunter Market Evidence v2 packages.")
