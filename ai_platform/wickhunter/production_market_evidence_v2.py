...
class CaptureRequestV2:
    ...
    def expected_sample_count(self) -> int:
        return (self.decision_end_ms - self.decision_start_ms) // (
            self.sample_interval_seconds * 1000
        )
...
def _write_ndjson(path: Path, rows: Sequence[Mapping[str, object]]) -> None:
    _write_new(path, b"".join(_canonical_bytes(dict(row)) + b"\n" for row in rows))
...
def _verify_self_hash(value: Mapping[str, object], *, hash_field: str, field: str) -> None:
...
def _require_zero(payload: Mapping[str, object], field: str) -> None:
    if payload.get(field) != 0:
        raise ProductionMarketEvidenceV2Error(f"{field} must equal zero")


def load_capture_request(path: Path) -> CaptureRequestV2:  # noqa: C901
    ...
    if not re.fullmatch(r"wickhunter-production-market-evidence-\d{8}-v2", request_id):
    ...
    if not re.fullmatch(
        r"wickhunter-production-market-evidence-\d{8}-v1-r1", base_v1_run_id
    ):
    ...
    if payload.get("profile") != "liquid20-v1" or payload.get("timeframe") != TIMEFRAME:
        raise ProductionMarketEvidenceV2Error("profile or timeframe mismatch")
    ...
    max_sample_lateness_seconds = _integer(
        payload.get("max_sample_lateness_seconds"), field="max_sample_lateness_seconds"
    )
    ...
    protected_holdout_start_ms = _integer(
        payload.get("protected_holdout_start_ms"), field="protected_holdout_start_ms"
    )
    ...
    _require_zero(payload, "orders_submitted")
    ...
    parsed_uri = urlsplit(request.durable_storage_uri)
    if (
...
def _instrument_rows_by_id(payload: Mapping[str, object]) -> dict[str, dict[str, Any]]:
    rows: dict[str, dict[str, Any]] = {}
    for index, raw in enumerate(_sequence(payload.get("data"), field="OKX instruments.data")):
...
def normalize_okx_instruments(
    ...
    *,
    ...
    try:
        contracts = parse_okx_instruments_response(root, requested_symbols=EXPECTED_SYMBOLS)
        ...
        records.append(
            {
                ...
                "quote_volume_24h_usd": _decimal_text(quote_volume),
...
def normalize_okx_candle_page(
    ...
    *,
    ...
    records.sort(key=lambda item: _integer(item["open_time_ms"], field="open_time_ms"))
    return records
...
def validate_okx_candle_coverage(
    ...
    *,
    ...
    actual = [_integer(row.get("open_time_ms"), field="open_time_ms") for row in records]
        ...
        for row in page:
            ...
            if start_ms <= open_ms < end_ms:
                ...
                if previous is not None and previous != row:
                    ...
                    comparable_previous.pop("fetched_at_ms", None)
                    comparable_row.pop("fetched_at_ms", None)
...
def _validate_public_url(url: str) -> None:
    ...
    if parsed.path.startswith(("/api/v5/account", "/api/v5/trade", "/api/v5/asset")):
...
def fetch_public_bytes(url: str, timeout_seconds: int = 30) -> bytes:
    ...
    request = Request(url, headers={"User-Agent": "freqtrade-wickhunter-okx-evidence/2"})
    try:
...
def _load_active(root: Path) -> tuple[Path, dict[str, Any]]:
    ...
    _verify_self_hash(pointer, hash_field="pointer_sha256", field="v2 active pointer")
    ...
    _verify_self_hash(state, hash_field="state_sha256", field="v2 incremental state")
    if state.get("run_id") != run_id or state.get("state_sha256") != pointer.get("state_sha256"):
...
def _sample_rows(run_root: Path) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    quality: list[dict[str, Any]] = []
    instruments: list[dict[str, Any]] = []
    for index in range(EXPECTED_SAMPLES):
        root = _sample_root(run_root, index)
        snapshot = _load_json(root / "market-snapshot.json", field=f"market snapshot {index}")
        instrument = _load_json(
            root / "instrument-snapshot.json", field=f"instrument snapshot {index}"
        )
        _verify_self_hash(snapshot, hash_field="snapshot_sha256", field=f"snapshot {index}")
        records = _sequence(snapshot.get("records"), field=f"snapshot {index} records")
        instrument_records = _sequence(
            instrument.get("records"), field=f"instrument snapshot {index} records"
        )
        if len(records) != len(EXPECTED_SYMBOLS) or len(instrument_records) != len(EXPECTED_SYMBOLS):
    ...
    *,
    ...
    with _exclusive_lock(durable_root):
        ...
        if state.get("status") != "active":
            return {"status": str(state.get("status")), "run_id": request.run_id}
        ...
        if now_ms < due_ms:
            return {"status": "waiting", "run_id": request.run_id, "due_ms": due_ms}
        ...
        if available_at_ms > latest_ms:
            raise ProductionMarketEvidenceV2Error("OKX responses completed after the lateness window")
        ...
        _write_json(
            sample_root / "source-health.json",
            {
                ...
                "required_scope": (
                    "ticker, spread, rolling quote volume, completed 5m candles, "
                    "instrument history"
                ),
...
def finalize_supplement(
    *,
    ...
    if _integer(state.get("next_sample_index"), field="next_sample_index") != EXPECTED_SAMPLES:
    ...
    try:
            ...
            candle_artifacts.append(
                {
                    ...
                    "start_ms": request.pre_roll_start_ms,
                    "end_ms": request.decision_end_ms,
                    "normalized_file": _artifact_identity(path, root=partial_root),
                }
            )
        _write_json(partial_root / "request.json", request.as_json_dict())
        _write_ndjson(partial_root / "source-snapshots.ndjson", source_rows)
        _write_ndjson(partial_root / "market-quality-observations.ndjson", quality_rows)
        _write_ndjson(partial_root / "instrument-snapshots.ndjson", instrument_rows)
        _write_json(
        ...
        artifacts = [_artifact_identity(partial_root / name, root=partial_root) for name in artifact_names]
        artifacts.extend(
            _artifact_identity(
                partial_root / str(item["normalized_file"]["logical_name"]),
                root=partial_root,
            )
        ...
        checksum_identities = [
            *artifacts,
            _artifact_identity(partial_root / "manifest.json", root=partial_root),
        ]
        _write_new(
        ...
        _write_json(partial_root / "verification-report.json", verification)
...
def verify_supplement(root: Path) -> dict[str, object]:  # noqa: C901
    ...
    if manifest.get("source") != OKX_SOURCE or manifest.get("symbols") != list(EXPECTED_SYMBOLS):
    ...
    for raw in artifacts:
        ...
        path = root / relative
        if path.is_symlink() or not path.is_file():
            raise ProductionMarketEvidenceV2Error("supplement artifact is missing")
        if _file_hash(path) != identity.get("sha256") or path.stat().st_size != identity.get(
            "size_bytes"
        ):
    ...
    expected_lines.add(f"{manifest_identity['sha256']}  manifest.json")
    ...
    verification = _load_json(root / "verification-report.json", field="verification report")
    if verification.get("outcome") != "accepted" or verification.get("manifest_sha256") != claimed:
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Capture and verify the public OKX Market Evidence v2 supplement.")
