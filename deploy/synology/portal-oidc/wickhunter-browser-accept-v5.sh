...
c=RequestContext(tenant_id="tenant-local",actor_id="system-wh09-browser-v5-preflight",actor_type=ActorType.SYSTEM,permissions=(Permission.BOT_READ,Permission.AUDIT_READ),request_id=uuid4(),correlation_id=uuid4(),causation_id=None)
...
PY
token_file="$RUNNER_TEMP/v5-token"; evidence="$RUNNER_TEMP/portal-wh09-browser-v5.json"; browser_name="portal-wh09-browser-v5-${GITHUB_RUN_ID}-${GITHUB_RUN_ATTEMPT}"
key="$(printf '%s:%s' "$GITHUB_RUN_ID" "$GITHUB_RUN_ATTEMPT"|sha256sum|awk '{print substr($1,1,24)}')"; principal_id="wh09-vp-$key"; membership_id="wh09-vm-$key"; subject="urn:freqtrade:wh09-browser-v5:${GITHUB_RUN_ID}:${GITHUB_RUN_ATTEMPT}"
...
for k,v in required.items():
 if e.get(k)!=v: raise SystemExit(f"browser v5 evidence mismatch: {k}")
