...
PY
runtime_contract="$(docker exec -i "$PORTAL_CONTROL_CONTAINER" python - <<'PY'
import json,os,time
...
c=RequestContext(tenant_id="tenant-local",actor_id="system-wh09-browser-v6-preflight",actor_type=ActorType.SYSTEM,permissions=(Permission.BOT_READ,Permission.AUDIT_READ),request_id=uuid4(),correlation_id=uuid4(),causation_id=None)
...
for n in range(12):
 ...
 if b.status_code==t.status_code==r.status_code==200:
  rows=[x for x in b.json() if x.get("bot_id")=="wickhunter"]; truth=t.json(); desired=truth.get("desired_generation") or {}; observed=truth.get("observed_generation") or {}; e=r.json().get("runtime") or {}
  ...
  generation_converged=bool(desired.get("generation_id")) and desired.get("generation_id")==observed.get("generation_id") and truth.get("pending_rollout") is False
  model_version=observed.get("model_version")
  if generation_converged and observed.get("managed_mode")=="shadow" and model_version and e.get("health")=="HEALTHY" and e.get("decision_count",0)>0 and e.get("no_trade_count",0)>0:
   print(json.dumps({"mode":"shadow","model_version":model_version,"desired_generation":desired["generation_id"],"observed_generation":observed["generation_id"]},sort_keys=True)); break
  last=f"health={e.get('health')} pending={truth.get('pending_rollout')} mode={observed.get('managed_mode')} model={model_version}"
...
readarray -t expected < <(python3 - "$runtime_contract" <<'PY'
import json,sys
p=json.loads(sys.argv[1]); print(p["mode"]); print(p["model_version"]); print(p["desired_generation"]); print(p["observed_generation"])
PY
)
expected_mode="${expected[0]}"; expected_model_version="${expected[1]}"; expected_desired_generation="${expected[2]}"; expected_observed_generation="${expected[3]}"
[[ "$expected_mode" == shadow && -n "$expected_model_version" && "$expected_desired_generation" == "$expected_observed_generation" ]]
token_file="$RUNNER_TEMP/v6-token"; evidence="$RUNNER_TEMP/portal-wh09-browser-v6.json"; browser_log="$RUNNER_TEMP/portal-wh09-browser-v6.log"; browser_name="portal-wh09-browser-v6-${GITHUB_RUN_ID}-${GITHUB_RUN_ATTEMPT}"
key="$(printf '%s:%s' "$GITHUB_RUN_ID" "$GITHUB_RUN_ATTEMPT"|sha256sum|awk '{print substr($1,1,24)}')"; principal_id="wh09-vp-$key"; membership_id="wh09-vm-$key"; subject="urn:freqtrade:wh09-browser-v6:${GITHUB_RUN_ID}:${GITHUB_RUN_ATTEMPT}"
...
try:
 with f() as s:
  r=IdentityRepository(s); p=r.create_principal(principal_id=os.environ["P"],issuer=cfg.issuer,subject=os.environ["S"],display_name="WH09 browser v6 acceptance",email=None,now=now); m=r.create_membership(membership_id=os.environ["M"],principal_id=p.principal_id,tenant_id="tenant-local",roles=(RoleName.USER,),valid_from=now,valid_until=now+timedelta(minutes=30),now=now); r.create_session(session_id_hash=cry.hash_token(os.environ["T"]),csrf_token_hash=cry.hash_token(os.environ["T"]+":csrf"),principal_id=p.principal_id,membership_id=m.membership_id,membership_version=m.membership_version,idp_session_id=None,authentication_time=now,mfa_satisfied=True,created_at=now,idle_expires_at=now+timedelta(minutes=15),absolute_expires_at=now+timedelta(minutes=30)); s.commit()
finally: eng.dispose()
PY
browser_rc=0
if docker create --name "$browser_name" --label "ftai.task=FTAI-20260819-wickhunter-portal-real-integration-1561" --label "ftai.harness_sha=$HARNESS_SHA" --label "ftai.target_authorization_sha=$TARGET_AUTHORIZATION_SHA" --read-only --tmpfs /tmp:rw,noexec,nosuid,nodev,size=512m --cap-drop ALL --security-opt no-new-privileges:true --shm-size 256m --env WICKHUNTER_BROWSER_ORIGIN="$PORTAL_ORIGIN" --env WICKHUNTER_EXPECTED_MODE="$expected_mode" --env WICKHUNTER_EXPECTED_MODEL_VERSION="$expected_model_version" --env WICKHUNTER_EXPECTED_DESIRED_GENERATION="$expected_desired_generation" --env WICKHUNTER_EXPECTED_OBSERVED_GENERATION="$expected_observed_generation" --env WICKHUNTER_SESSION_TOKEN="$session_token" --env WICKHUNTER_CSRF_TOKEN="${session_token}:csrf" --env WICKHUNTER_BROWSER_EXECUTABLE_PATH=/usr/bin/chromium --env WICKHUNTER_BROWSER_NO_SANDBOX=1 "$BROWSER_IMAGE" >/dev/null; then
 set +e
 docker start --attach "$browser_name" >"$browser_log" 2>&1
 browser_rc=$?
 set -e
 cat "$browser_log"
 python3 - "$browser_log" "$evidence" <<'PY'
import json,pathlib,sys
prefix="WICKHUNTER_EVIDENCE="; rows=[line[len(prefix):] for line in pathlib.Path(sys.argv[1]).read_text().splitlines() if line.startswith(prefix)]
if rows:
 payload=json.loads(rows[-1]); pathlib.Path(sys.argv[2]).write_text(json.dumps(payload,indent=2,sort_keys=True)+"\n")
PY
else browser_rc=$?; fi
cleanup_rc=0
...
PY
rm -f "$token_file" "$browser_log"; resource_rc=0; docker rm -f "$browser_name" >/dev/null 2>&1 || true; docker image rm "$BROWSER_IMAGE" >/dev/null 2>&1 || resource_rc=$?
[[ "$cleanup_rc" -eq 0 && "$resource_rc" -eq 0 ]]; test -s "$evidence"
python3 - "$evidence" "$HARNESS_SHA" "$TARGET_AUTHORIZATION_SHA" "$expected_model_version" "$expected_desired_generation" "$expected_observed_generation" <<'PY'
import json,sys
p=sys.argv[1]; e=json.load(open(p)); required={"result":"PASS","origin":"https://quant.molehill.cloud","mode":"shadow","authenticated":True,"fixture_cookie_present":False,"health_visible":True,"runtime_generation_converged":True,"reload_persistence":True,"expected_model_version":sys.argv[4],"expected_desired_generation":sys.argv[5],"expected_observed_generation":sys.argv[6]}
for k,v in required.items():
 if e.get(k)!=v: raise SystemExit(f"browser v6 evidence mismatch: {k}")
if e.get("decision_count",0)<=0 or e.get("no_trade_count",0)<=0: raise SystemExit("missing decision evidence")
e.update({"harness_source_sha":sys.argv[2],"target_authorization_sha":sys.argv[3],"trading_credentials_present":False,"order_adapter_present":False,"execution_enabled":False,"orders_submitted":0,"live_capital_authorized":False}); open(p,"w").write(json.dumps(e,indent=2,sort_keys=True)+"\n")
PY
[[ "$browser_rc" -eq 0 ]]
