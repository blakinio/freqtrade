...
EVIDENCE="$RUNNER_TEMP/issue1396-trusted-transition"
RECOVERY="$EVIDENCE/recovery"
PROOF_DIR="$EVIDENCE/proof"
mkdir -p "$EVIDENCE" "$RECOVERY" "$PROOF_DIR"

log() { printf '[issue1396-trusted-transition] %s\n' "$*"; }
...
validate_generation_snapshot() {
  ...
  local output_path="$3"
  python3 - "$health_path" "$journal_root" "$output_path" "$IMPLEMENTATION_SHA" <<'PY'
import json,sys
from pathlib import Path
health=json.load(open(sys.argv[1],encoding='utf-8'))
root=Path(sys.argv[2]); out=Path(sys.argv[3]); implementation=sys.argv[4]
...
if health.get('operator_commit')!=implementation: raise SystemExit('persistent PAPER operator commit mismatch')
...
expected=['binance-usdm','bybit-linear','okx-swap']; sources=s.get('source_freshness')
...
if any(item.get('health')!='healthy' or item.get('fresh') is not True or not isinstance(item.get('age_ms'),int) or item['age_ms']>300000 for item in sources): raise SystemExit('persistent source unhealthy or stale')
for key,value in zero.items():
...
recover() {
  ...
  if [[ -z "${OLD_SHADOW_CONTAINER_ID:-}" ]]; then
    printf '%s\n' '{"result":"NO_RUNTIME_MUTATION_BEFORE_SHADOW_IDENTITY"}' > "$RECOVERY/result.json"
    return 0
  fi
  if ! docker container inspect "$PROD_PAPER_CONTAINER" >/dev/null 2>&1; then
    if [[ "${OLD_SHADOW_WAS_RUNNING:-false}" == "true" ]]; then docker start "$OLD_SHADOW_CONTAINER_ID" >/dev/null 2>&1 || true; fi
    printf '%s\n' '{"result":"NO_PAPER_RUNTIME_CREATED"}' > "$RECOVERY/result.json"
    return 0
  fi
  local paper_id health_file result recovery_exit
  ...
  if [[ "$(docker inspect --format '{{.State.Health.Status}}' "$PROD_PAPER_CONTAINER" 2>/dev/null)" == "healthy" && -f "$health_file" && "$(docker inspect --format '{{.State.Running}}' "$PORTAL_CONTROL_CONTAINER" 2>/dev/null)" == "true" ]]; then
    cp "$REQUEST_HELPER" "$RECOVERY/issue1396_trusted_terminal_transition.py"
    cp "$health_file" "$RECOVERY/paper-health.json"
    docker cp "$RECOVERY/issue1396_trusted_terminal_transition.py" "$PORTAL_CONTROL_CONTAINER:/tmp/issue1396_trusted_terminal_transition.py" >/dev/null 2>&1
    docker cp "$RECOVERY/paper-health.json" "$PORTAL_CONTROL_CONTAINER:/tmp/issue1396-recovery-paper-health.json" >/dev/null 2>&1
    docker exec "$PORTAL_CONTROL_CONTAINER" python /tmp/issue1396_trusted_terminal_transition.py recover-to-paper \
      --old-runtime-instance-id "$OLD_SHADOW_CONTAINER_ID" \
      --paper-runtime-instance-id "$paper_id" \
      --health-json /tmp/issue1396-recovery-paper-health.json \
      --source-version "$IMPLEMENTATION_SHA" \
      --image-digest "${PAPER_IMAGE_DIGEST:-missing}" \
      --config-digest "${PAPER_CONFIG_DIGEST:-missing}" > "$RECOVERY/result.json"
    recovery_exit=$?
...
log "bind accepted trusted 5/5 PAPER artifact"
META="$EVIDENCE/proof-artifact-meta.json"; ZIP="$EVIDENCE/proof.zip"
...
python3 - "$PROOF_DIR/issue1396-proof.json" "$PROOF_DIR/issue1396-liquid20.json" "$PROOF_DIR/issue1396-activation.json" "$IMPLEMENTATION_SHA" "$PROOF_PAPER_IMAGE_DIGEST" "$EXPECTED_CANDIDATE_MANIFEST_SHA256" <<'PY'
import json,re,sys
proof,pre,activation=[json.load(open(x,encoding='utf-8')) for x in sys.argv[1:4]]
implementation,image,manifest=sys.argv[4:7]
zero={'protected_holdout_accessed':False,'automatic_promotion_enabled':False,'trading_credentials_present':False,'order_adapter_present':False,'execution_enabled':False,'orders_submitted':0,'live_capital_authorized':False}
if proof.get('result')!='PASS' or proof.get('implementation_sha')!=implementation or proof.get('image_digest')!=image: raise SystemExit('trusted proof identity mismatch')
if proof.get('observation_count')!=5 or proof.get('telemetry_record_count')!=5 or proof.get('successful_outcomes')!=5: raise SystemExit('trusted proof is not exact 5/5')
try: ratio=float(proof.get('fresh_source_ratio'))
except (TypeError,ValueError): raise SystemExit('trusted proof freshness is invalid')
if ratio<0.99 or proof.get('runtime_health')!='healthy': raise SystemExit('trusted proof health/freshness failed')
for key,value in zero.items():
    if proof.get(key)!=value: raise SystemExit(f'trusted proof authority mismatch: {key}')
generations=proof.get('generation_health')
if not isinstance(generations,list) or len(generations)!=5 or [x.get('generation') for x in generations]!=[1,2,3,4,5]: raise SystemExit('trusted proof generation set mismatch')
expected=['binance-usdm','bybit-linear','okx-swap']
for row in generations:
    sources=row.get('sources')
    if row.get('health')!='healthy' or not isinstance(sources,list) or sorted(x.get('source') for x in sources)!=expected: raise SystemExit('trusted generation health mismatch')
    if any(x.get('health')!='healthy' or x.get('fresh') is not True or not isinstance(x.get('age_ms'),int) or x['age_ms']>300000 for x in sources): raise SystemExit('trusted generation source unhealthy')
if pre.get('result')!='PASS' or sorted(x.get('source') for x in pre.get('sources',[]))!=expected: raise SystemExit('trusted Liquid20 preflight mismatch')
if any(x.get('health')!='healthy' or x.get('coverage_available') is not True or not isinstance(x.get('age_ms'),int) or x['age_ms']>300000 for x in pre['sources']): raise SystemExit('trusted Liquid20 source unhealthy')
if activation.get('candidate_manifest_sha256')!=manifest: raise SystemExit('trusted activation candidate mismatch')
for key,value in {'automatic_promotion_enabled':False,'trading_credentials_present':False,'order_adapter_present':False,'execution_enabled':False,'orders_submitted':0,'live_capital_authorized':False}.items():
    if activation.get(key)!=value: raise SystemExit(f'trusted activation authority mismatch: {key}')
if not re.fullmatch(r'[0-9a-f]{64}',proof.get('binding_id','')) or not re.fullmatch(r'[0-9a-f]{64}',proof.get('run_id','')): raise SystemExit('trusted proof binding identity invalid')
PY

log "build exact proven PAPER image and reverify current Liquid20"
...
image_id="$(docker image inspect --format '{{.Id}}' "$PAPER_IMAGE")"; [[ "$image_id" == "sha256:$PROOF_PAPER_IMAGE_DIGEST" ]]
...
docker run --rm -i --user 65532:65532 --group-add "$LIQUID20_READER_GID" --read-only --network none --cap-drop ALL --security-opt no-new-privileges:true \
...
log "capture legacy SHADOW identity without mutating it"
...
docker inspect "$OLD_SHADOW_CONTAINER_ID" > "$EVIDENCE/old-shadow-before.json"
OLD_SHADOW_WAS_RUNNING="$(python3 - "$EVIDENCE/old-shadow-before.json" <<'PY'
import json,sys
p=json.load(open(sys.argv[1],encoding='utf-8'))[0]
labels=(p.get('Config') or {}).get('Labels') or {}; host=p.get('HostConfig') or {}
...
print('true' if (p.get('State') or {}).get('Running') is True else 'false')
PY
)"; export OLD_SHADOW_WAS_RUNNING

log "install checksum-pinned Portal supply-chain tools and deploy exact current Portal"
...
PORTAL_EVIDENCE="$EVIDENCE/portal-supply-chain"; APPROVAL="$PORTAL_EVIDENCE/approval.json"; REPORT="$EVIDENCE/portal-deploy-report.json"
DEPLOY_REQUEST="$GITHUB_WORKSPACE/deploy/synology/portal-oidc/run-requests/public-oidc-20260801-v1.json"
...
cp "$REQUEST_HELPER" "$EVIDENCE/issue1396_trusted_terminal_transition.py"
docker cp "$EVIDENCE/issue1396_trusted_terminal_transition.py" "$PORTAL_CONTROL_CONTAINER:/tmp/issue1396_trusted_terminal_transition.py"
docker exec "$PORTAL_CONTROL_CONTAINER" python -m py_compile /tmp/issue1396_trusted_terminal_transition.py

log "materialize immutable persistent PAPER activation and gateway identities"
...
PY
GATEWAY_CODE_SHA256="$(sha256sum "$EVIDENCE/gateway.py" | awk '{print $1}')"; export GATEWAY_CODE_SHA256
python3 - "$EVIDENCE/gateway-material.json" "$IMPLEMENTATION_SHA" "$PAPER_IMAGE_DIGEST" "$GATEWAY_CODE_SHA256" "$PROD_INTERNAL_NETWORK" <<'PY'
import json,pathlib,sys
from ai_platform.wickhunter.canonical import canonical_sha256
path,implementation,image,code,network=sys.argv[1:]
artifact={'schema_version':'wickhunter-public-market-gateway-artifact-v3','implementation_sha':implementation,'runtime_image_digest':image,'gateway_code_sha256':code,'target':'fapi.binance.com:443'}
contract={'schema_version':'wickhunter-public-market-gateway-contract-v3','market_data_only':True,'target':'fapi.binance.com:443','tls_passthrough':True,'credentials_allowed':False,'orders_allowed':False}
egress={'schema_version':'binance-usdm-public-market-only-internal-gateway-v3','internal_network':network,'allowed_target':'fapi.binance.com:443','direct_external_egress':False,'trading_credentials_present':False,'order_adapter_present':False}
payload={'gateway_artifact_digest':canonical_sha256(artifact),'gateway_contract_digest':canonical_sha256(contract),'market_egress_policy_digest':canonical_sha256(egress),'artifact':artifact,'contract':contract,'egress':egress}
pathlib.Path(path).write_text(json.dumps(payload,sort_keys=True)+'\n',encoding='utf-8')
PY
GATEWAY_ARTIFACT_DIGEST="$(json_value "$EVIDENCE/gateway-material.json" gateway_artifact_digest)"; GATEWAY_CONTRACT_DIGEST="$(json_value "$EVIDENCE/gateway-material.json" gateway_contract_digest)"; MARKET_EGRESS_POLICY_DIGEST="$(json_value "$EVIDENCE/gateway-material.json" market_egress_policy_digest)"
export GATEWAY_ARTIFACT_DIGEST GATEWAY_CONTRACT_DIGEST MARKET_EGRESS_POLICY_DIGEST
docker run --rm -i --user 65532:65532 --read-only --network none --cap-drop ALL --security-opt no-new-privileges:true \
  ...
  --env GATEWAY_ARTIFACT_DIGEST --env GATEWAY_CONTRACT_DIGEST --env MARKET_EGRESS_POLICY_DIGEST \
...
if CandidatePaperRuntimeService(binding=binding,runtime_policy=_runtime_policy(),journal_root=journal).journal.latest_state() is not None: raise SystemExit('production PAPER journal is not fresh')
auth={'schema_version':'wickhunter-issue1396-paper-authorization-v4','authorization_id':'issue-1396-production-paper-owner-directed-v4','run_id':binding.request.run_id,'binding_id':binding.binding_id,'candidate_package_id':result.identity.package_id,'candidate_manifest_sha256':result.identity.manifest_sha256,'live_capital_authorized':False,'execution_enabled':False}
...
find "$journal_runner" -type d -exec chmod 0750 {} +; find "$journal_runner" -type f -exec chmod 0640 {} +; chown -R 65532:65532 "$journal_runner" "$operator_runner"
...
log "start persistent PAPER and prove exact identity before desired mutation"
...
docker create --name "$PROD_GATEWAY_CONTAINER" --init --user 0:0 --read-only --restart unless-stopped --tmpfs /tmp:rw,noexec,nosuid,nodev,size=16m,mode=1777 --cap-drop ALL --cap-add NET_BIND_SERVICE --security-opt no-new-privileges:true --memory 256m --network bridge \
  --label "io.freqtrade.wickhunter.issue=1396" --label "io.freqtrade.wickhunter.role=production-paper-public-market-egress" --label "io.freqtrade.wickhunter.target=fapi.binance.com:443" --label "io.freqtrade.wickhunter.gateway-artifact-digest=$GATEWAY_ARTIFACT_DIGEST" --label "org.opencontainers.image.revision=$IMPLEMENTATION_SHA" \
...
docker create --name "$PROD_PAPER_CONTAINER" --restart unless-stopped \
  --label "io.freqtrade.wickhunter.issue=1396" --label "io.freqtrade.wickhunter.bot=wickhunter" --label "io.freqtrade.wickhunter.mode=paper" --label "io.freqtrade.wickhunter.binding=$PAPER_BINDING_ID" --label "io.freqtrade.wickhunter.config-digest=$PAPER_CONFIG_DIGEST" --label "org.opencontainers.image.revision=$IMPLEMENTATION_SHA" \
  --user 65532:65532 --group-add "$LIQUID20_READER_GID" --read-only --tmpfs /tmp:rw,noexec,nosuid,nodev,size=64m,mode=1777 --cap-drop ALL --security-opt no-new-privileges:true --memory 2g --cpus 2.0 --pids-limit 256 \
  --network "$PROD_INTERNAL_NETWORK" --add-host "fapi.binance.com:$gateway_ip" \
  --env "OPERATOR_COMMIT=$IMPLEMENTATION_SHA" --env HEALTH_PATH=/runtime/operator/health.json --env HEALTH_MAX_AGE_SECONDS=1200 --env HTTP_PROXY= --env HTTPS_PROXY= --env ALL_PROXY= --env http_proxy= --env https_proxy= --env all_proxy= \
  --mount "type=bind,src=$candidate_host,dst=/runtime/candidate,readonly" --mount "type=bind,src=$PROD_ACTIVATION_HOST,dst=/runtime/activation,readonly" --mount "type=bind,src=$LIQUID20_LIVE_HOST,dst=/runtime/liquid20,readonly" --mount "type=bind,src=$PROD_JOURNAL_HOST,dst=/runtime/journal" --mount "type=bind,src=$PROD_OPERATOR_HOST,dst=/runtime/operator" \
  --health-cmd 'python /app/deploy/synology/wickhunter-paper-runtime/paper_runtime_healthcheck.py' --health-interval 30s --health-timeout 10s --health-retries 3 --health-start-period 120s \
  "$PAPER_IMAGE" --candidate-root /runtime/candidate --activation-root /runtime/activation --journal-root /runtime/journal --liquid20-root /runtime/liquid20 --health-root /runtime/operator --operator-commit "$IMPLEMENTATION_SHA" --public-market-base-url https://fapi.binance.com --poll-seconds 120 --maximum-source-age-ms 300000 --model-drift healthy --data-drift healthy --circuit-breaker-active false >/dev/null
...
[[ "$(docker inspect --format '{{index .Config.Labels "io.freqtrade.wickhunter.config-digest"}}' "$PROD_PAPER_CONTAINER")" == "$PAPER_CONFIG_DIGEST" ]]
[[ "$(docker inspect --format '{{index .Config.Labels "io.freqtrade.wickhunter.gateway-artifact-digest"}}' "$PROD_GATEWAY_CONTAINER")" == "$GATEWAY_ARTIFACT_DIGEST" ]]
docker inspect "$PROD_PAPER_CONTAINER" > "$EVIDENCE/production-paper-inspect.json"
python3 - "$EVIDENCE/production-paper-inspect.json" "$PROD_INTERNAL_NETWORK" <<'PY'
import json,sys
p=json.load(open(sys.argv[1],encoding='utf-8'))[0]; host=p.get('HostConfig') or {}; config=p.get('Config') or {}; mounts={x.get('Destination'):x for x in p.get('Mounts') or []}
if config.get('User')!='65532:65532' or host.get('ReadonlyRootfs') is not True or host.get('Privileged') is not False: raise SystemExit('PAPER hardening identity mismatch')
if 'ALL' not in (host.get('CapDrop') or []) or not any('no-new-privileges' in x for x in (host.get('SecurityOpt') or [])): raise SystemExit('PAPER capability hardening mismatch')
if sys.argv[2] not in ((p.get('NetworkSettings') or {}).get('Networks') or {}): raise SystemExit('PAPER internal network mismatch')
for destination in ('/runtime/candidate','/runtime/activation','/runtime/liquid20'):
    m=mounts.get(destination)
    if not isinstance(m,dict) or m.get('Type')!='bind' or m.get('RW') is not False: raise SystemExit(f'PAPER read-only mount mismatch: {destination}')
for destination in ('/runtime/journal','/runtime/operator'):
    m=mounts.get(destination)
    if not isinstance(m,dict) or m.get('Type')!='bind' or m.get('RW') is not True: raise SystemExit(f'PAPER writable evidence mount mismatch: {destination}')
print(json.dumps({'result':'PASS','runtime_instance_id':p['Id'],'hardening':'matched'},sort_keys=True))
PY
...
log "physically stop and bind legacy SHADOW only after PAPER health is proven"
if [[ "$(docker inspect --format '{{.State.Running}}' "$OLD_SHADOW_CONTAINER_ID")" == "true" ]]; then docker stop --time 30 "$OLD_SHADOW_CONTAINER_ID" >/dev/null; fi
[[ "$(docker inspect --format '{{.State.Running}}' "$OLD_SHADOW_CONTAINER_ID")" == "false" ]]
docker inspect "$OLD_SHADOW_CONTAINER_ID" > "$EVIDENCE/old-shadow-stopped-inspect.json"
python3 - "$EVIDENCE/old-shadow-stopped-inspect.json" > "$EVIDENCE/old-shadow.json" <<'PY'
import json,sys
p=json.load(open(sys.argv[1],encoding='utf-8'))[0]; state=p.get('State') or {}; image=p.get('Image','')
...
docker exec "$PORTAL_CONTROL_CONTAINER" python /tmp/issue1396_trusted_terminal_transition.py baseline-stopped --old-container-id "$OLD_SHADOW_CONTAINER_ID" --old-image-digest "$OLD_SHADOW_IMAGE_DIGEST" > "$EVIDENCE/baseline.json"

log "author desired PAPER and reconcile exact stopped/running physical truth"
docker cp "$EVIDENCE/production-health.json" "$PORTAL_CONTROL_CONTAINER:/tmp/issue1396-production-health.json"
docker exec "$PORTAL_CONTROL_CONTAINER" python /tmp/issue1396_trusted_terminal_transition.py author-paper \
  --implementation-sha "$IMPLEMENTATION_SHA" --image-digest "$PAPER_IMAGE_DIGEST" --config-digest "$PAPER_CONFIG_DIGEST" \
  --authorization-id "$PAPER_AUTHORIZATION_ID" --authorization-digest "$PAPER_AUTHORIZATION_DIGEST" --candidate-package-id "$PAPER_CANDIDATE_PACKAGE_ID" --candidate-manifest "$PAPER_CANDIDATE_MANIFEST" \
  --internal-network "$PROD_INTERNAL_NETWORK" --gateway-artifact-digest "$GATEWAY_ARTIFACT_DIGEST" --gateway-contract-digest "$GATEWAY_CONTRACT_DIGEST" --market-egress-policy-digest "$MARKET_EGRESS_POLICY_DIGEST" > "$EVIDENCE/desired-paper.json"
...
docker exec "$PORTAL_CONTROL_CONTAINER" python /tmp/issue1396_trusted_terminal_transition.py record-stop --generation-id "$OLD_GENERATION_ID" --runtime-instance-id "$OLD_SHADOW_CONTAINER_ID" --evidence-kind issue1396-physical-stopped-shadow-v4 > "$EVIDENCE/shadow-stopped.json"
docker exec "$PORTAL_CONTROL_CONTAINER" python /tmp/issue1396_trusted_terminal_transition.py reconcile-running --generation-id "$PAPER_GENERATION_ID" --runtime-instance-id "$PROD_PAPER_CONTAINER_ID" --health-json /tmp/issue1396-production-health.json --source-version "$IMPLEMENTATION_SHA" --evidence-kind issue1396-production-paper-running-v4 --actor-id system-issue1396-paper-reconciler-v4 > "$EVIDENCE/paper-reconciled.json"

log "restart exact PAPER runtime and reconcile a newer observation"
...
for _ in $(seq 1 90); do
  ...
  if [[ "$h" == "healthy" && -f "$PROD_OPERATOR_HOST/health.json" ]]; then
    ...
    c="$(python3 -c 'import json,sys;print(json.load(open(sys.argv[1]))["checked_at_ms"])' "$PROD_OPERATOR_HOST/health.json")"
    if (( g > before_generation && c > before_checked )); then break; fi
  fi
  [[ "$h" =~ ^(unhealthy|exited|dead)$ ]] && { docker logs "$PROD_PAPER_CONTAINER"; exit 1; }
  sleep 10
done
...
python3 - "$EVIDENCE/restart-proof.json" "$same_id" "$before_generation" "$after_generation" "$PAPER_IMAGE_DIGEST" "$PAPER_CONFIG_DIGEST" <<'PY'
import json,pathlib,sys
path,runtime,before,after,image,config=sys.argv[1:]
p={'result':'PASS','same_runtime_instance':True,'runtime_instance_id':runtime,'before_generation':int(before),'after_generation':int(after),'paper_image_digest':image,'paper_config_digest':config}
...
docker exec "$PORTAL_CONTROL_CONTAINER" python /tmp/issue1396_trusted_terminal_transition.py reconcile-running --generation-id "$PAPER_GENERATION_ID" --runtime-instance-id "$PROD_PAPER_CONTAINER_ID" --health-json /tmp/issue1396-production-health-after-restart.json --source-version "$IMPLEMENTATION_SHA" --evidence-kind issue1396-production-paper-restart-v4 --actor-id system-issue1396-paper-restart-reconciler-v4 > "$EVIDENCE/paper-restart-observation.json"

log "capture real canonical Portal API truth and closeout payload"
docker exec "$PORTAL_CONTROL_CONTAINER" python /tmp/issue1396_trusted_terminal_transition.py final-api --expected-mode paper > "$EVIDENCE/production-api.json"
python3 - "$EVIDENCE/production-api.json" "$PROOF_DIR/issue1396-proof.json" "$EVIDENCE/production-health-after-restart.json" "$EVIDENCE/restart-proof.json" "$EVIDENCE/old-shadow.json" "$EVIDENCE/production-activation.json" "$EVIDENCE/gateway-material.json" "$PROOF_ARTIFACT_ID" "$PROOF_ARTIFACT_DIGEST" "$PROOF_RUN_ID" "$IMPLEMENTATION_SHA" "$PAPER_IMAGE_DIGEST" "$PAPER_CONFIG_DIGEST" "$PROD_PAPER_CONTAINER_ID" > "$EVIDENCE/production-closeout.json" <<'PY'
import json,sys
api,proof,health,restart,old,activation,gateway=[json.load(open(x,encoding='utf-8')) for x in sys.argv[1:8]]
artifact_id,artifact_digest,proof_run,implementation,image,config,paper_id=sys.argv[8:15]
...
for key,value in zero.items():
    if key in health and health.get(key)!=value: raise SystemExit(f'final authority mismatch: {key}')
payload={'result':'PASS','implementation_sha':implementation,'bot':bot,'truth':truth,'five_cycle_proof':proof,'proof_artifact':{'id':int(artifact_id),'sha256':artifact_digest,'run_id':int(proof_run)},'paper_health':health,'restart':restart,'physical':{'paper_container_id':paper_id,'old_shadow_container_id':old['container_id'],'paper_image_digest':image,'paper_config_digest':config},'activation':activation,'gateway':gateway,'zero_authority':zero}
print(json.dumps(payload,sort_keys=True))
PY
DESIRED_GENERATION_ID="$(json_value "$EVIDENCE/production-closeout.json" truth.desired_generation.generation_id)"; OBSERVED_GENERATION_ID="$(json_value "$EVIDENCE/production-closeout.json" truth.observed_generation.generation_id)"; [[ "$DESIRED_GENERATION_ID" == "$OBSERVED_GENERATION_ID" ]]
base64 -w0 "$EVIDENCE/production-closeout.json" > "$EVIDENCE/production-closeout.b64"
{
  ...
  echo "paper_state_runner=$PROD_STATE_RUNNER"
} >> "$GITHUB_OUTPUT"

log "trusted terminal transition passed"
