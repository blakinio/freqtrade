...
Before treating the absence of Codex or a local terminal as a blocker, read and follow `GITHUB_ONLY_EXECUTION.md`. Use the GitHub connection and GitHub Actions on a dedicated branch, select the smallest proving validation, inspect full failed-job logs, keep repairs bounded, preserve required artifacts, and report an exact technical blocker only after the contract's alternatives are exhausted. This does not authorize merge, auto-merge, production deployment, live-capital interaction, secret exposure, or protection bypass.
...
11. Do not call a user-facing capability complete while any required backend, frontend, integration or consumer layer is missing.
