...
Paste the complete text below into a separate agent chat only when the closure dispatch table authorizes `FTAI-20260730-closure-signal-wizard-backend`.
...
Pracuj na `agent/closure-signal-wizard-backend`, utworzonym z aktualnego `develop` dopiero po ponownym preflightcie.
...
- trwały preview store oraz trwały experiment-admission submit store;
...
- service, API, persistence, compatibility, tenant-isolation, idempotency i secret-exclusion tests.
