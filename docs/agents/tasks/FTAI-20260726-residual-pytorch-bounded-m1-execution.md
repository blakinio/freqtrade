...
- use download coverage ending at the same exclusive `20260501` boundary and never read May-June consumed historical OOS;
...
- access consumed historical OOS `20260501-20260630` or protected final holdout `20260801-20260930`;
...
proven:
  ...
  - P2 fixed target semantics at future offsets t+1 through t+12 and left exact expanded feature and historical distribution evidence unresolved.
  - Existing guarded execution infrastructure proves the one-shot exact-one-file trigger pattern and Kraken historical-data path.
  - The current P3 authorization excludes consumed May-June historical OOS and the protected final holdout.
derived:
  - P3 must use March-April historical development evidence and cannot describe it as fresh strict OOS.
  - Historical matrix auditing is a mandatory fail-closed prerequisite inside P3, not an inferred P2 result.
unknown:
  - Exact expanded feature count and historical NaN, outlier and target distributions on authorized data.
  - Whether all three frozen models complete the bounded historical-development lifecycle on current develop.
conflicts: []
first_failure:
  marker: none
  evidence: No P3 implementation or execution has started.
rejected_hypotheses:
...
validation: []
blockers: []
next_action: Merge this declaration-only task after checkpoint and repository gates pass, then implement inert fail-closed P3 infrastructure on a separate branch without adding or executing the canonical run request.
```
