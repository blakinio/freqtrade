...
def test_tenant_mismatch_fails_before_compatibility_evaluation() -> None:
    service = build_service()
    access = build_access()
    mismatched = build_selection().model_copy(update={"tenant_id": "tenant-b"})
...
def test_compatibility_requires_catalog_read_capability() -> None:
    service = build_service()
    selection = build_selection()
...
def test_stale_template_revision_has_stable_reason_and_missing_proof() -> None:
    service = build_service()
    access = build_access()
    stale = build_selection().model_copy(
...
def test_required_model_is_rejected_when_selection_omits_it() -> None:
    access = build_access()
    ...
    without_model = build_selection().model_copy(update={"model_version": None})
...
def test_deprecated_model_rejects_with_stale_evidence() -> None:
...
def test_missing_model_has_model_and_evidence_reasons() -> None:
...
def test_cross_capability_conflicts_return_sorted_reason_codes() -> None:
    snapshot = snapshot_with_templates((template_entry(),))
    service = BotCatalogService(InMemoryBotCatalogRepository((snapshot,)))
    incompatible = build_selection().model_copy(
        update={
    ...
    decision = service.decide_compatibility(build_access(), snapshot.catalog_ref, incompatible, NOW)
...
def test_latest_catalog_ref_is_capability_gated() -> None:
    assert build_service().latest_catalog_ref(
        build_access(),
        "approved-bots",
    ) == CatalogVersionRef(
