...
class _Verifier:
    ...
    def submit(self, target, submission, lease):
        raise AssertionError("BM-07 verifier must not submit PI-08 orders")
...
class _Transport:
    ...
    def force_exit(self, target, lease, *, trade_id: str, amount: str | None = None):
        del target, lease
    ...
    def cancel_open_order(self, target, lease, *, trade_id: str):
        del target, lease
...
def _confirmation() -> CommandConfirmation:
    return CommandConfirmation(
        reason="operator confirmed",
        confirmed_at=NOW,
        step_up_session_id="step-up-1",
    )
...
def test_partial_close_is_reserved_before_runtime_and_replay_does_not_repeat_io(
    tmp_path: Path,
) -> None:
...
def test_price_changing_replace_and_cross_tenant_evidence_fail_closed() -> None:
    price_command = OrderCommand.model_validate(
        {
            **_order_command(OrderAction.REPLACE_ORDER).model_dump(),
            "replacement_quantity": None,
            "replacement_price": Decimal("65000"),
        }
    )
...
def test_dca_and_grid_entries_delegate_to_single_pi08_path(tmp_path: Path) -> None:
    ...
    receipt = service.activate_policy_entry(request)
