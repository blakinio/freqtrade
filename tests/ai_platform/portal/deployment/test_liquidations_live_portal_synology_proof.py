...
def test_script_uses_explicit_fixture_identity_only_in_isolated_candidate() -> None:
    ...
    assert '"authenticated_with_fixture_session": True' in text
...
def test_script_records_candidate_runtime_security_and_fail_closed_result() -> None:
    ...
    assert '"read_only_root_filesystem": True' in text
    assert '"cap_drop_all": True' in text
    assert '"no_new_privileges": True' in text
    assert '"tmpfs_only_for_required_paths": True' in text
    assert '"memory_limit_bytes": 805306368' in text
