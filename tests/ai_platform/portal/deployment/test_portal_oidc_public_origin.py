...
def test_production_redirect_origin_is_not_derived_from_container_request() -> None:
    ...
    assert "safeBackendReturnLocation(upstream.headers.get(\"location\"), redirectOrigin)" in callback
    assert "safeBackendReturnLocation(upstream.headers.get(\"location\"), request.nextUrl.origin)" not in callback
