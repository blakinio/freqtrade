...
WORKFLOW_PATH = (
    REPO_ROOT / ".github/workflows/experimental-model-historical-backtest-execution.yml"
)
...
def test_canonical_request_pins_exact_two_isolated_tracks_and_boundaries() -> None:
    ...
    assert (
        request["protected_final_holdout"]
        == EXPECTED_PROTECTED_FINAL_HOLDOUT["timerange"]
    )
...
def test_request_rejects_track_input_hash_drift(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        ExperimentalModelHistoricalBacktestRunRequestError, match="tracks"
    ):
        load_experimental_model_historical_backtest_run_request(request_path)
...
def test_request_rejects_final_holdout_authorization(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        ExperimentalModelHistoricalBacktestRunRequestError, match="authorization"
    ):
        load_experimental_model_historical_backtest_run_request(request_path)
...
def test_request_rejects_extra_fields(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        ExperimentalModelHistoricalBacktestRunRequestError, match="extra=notes"
    ):
        load_experimental_model_historical_backtest_run_request(request_path)
