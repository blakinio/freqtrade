...
def test_synology_preflight_requires_okx_candidate_and_production_state() -> None:
    deploy = (ROOT / "deploy/synology/liquid20/deploy-live.sh").read_text(encoding="utf-8")
    required_sources = '("bybit-linear", "binance-usdm", "okx-swap")'
    required_files = 'for name in ("bybit-linear.ndjson", "binance-usdm.ndjson", "okx-swap.ndjson")'
...
def test_synology_runtime_refuses_okx_credentials_and_preserves_data_root() -> None:
    ...
    compose = (ROOT / "deploy/synology/liquid20/compose.yaml").read_text(encoding="utf-8")
