...
def _cases(*, include_holdout: bool = False) -> tuple[_FakeCase, ...]:
    ...
    if include_holdout:
        values.append(
            _FakeCase("holdout", canonical_sha256({"split": "holdout"}))
        )
    return tuple(values)
...
def test_search_is_seeded_bounded_and_input_order_independent() -> None:
    ...
    assert sum(
        trial.selection_kind is TrialSelectionKind.INITIAL
        for trial in forward.trials
    ) == 2
    assert sum(
        trial.selection_kind is TrialSelectionKind.SURROGATE
        for trial in forward.trials
    ) == 2
...
def test_result_retains_fail_closed_authority_boundary() -> None:
    ...
    assert all(
        trial.test_evidence is not None
        for trial in result.trials
        if trial.selected_top_k
    )
    assert all(
        trial.test_evidence is None
        for trial in result.trials
        if not trial.selected_top_k
    )
