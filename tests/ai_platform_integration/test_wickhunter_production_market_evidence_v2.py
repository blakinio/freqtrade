...
def test_load_request_preserves_v1_identity_and_requires_exact_three_sources(tmp_path: Path) -> None:
    request_path = tmp_path / "request.json"
    request_path.write_text(json.dumps(_request_payload(tmp_path / "durable")), encoding="utf-8")
    ...
    with pytest.raises(subject.ProductionMarketEvidenceV2Error, match="exact three-source"):
...
def test_initialize_refuses_credentials_and_proxy_routing(tmp_path: Path) -> None:
    ...
    request_path.write_text(json.dumps(_request_payload(durable_root)), encoding="utf-8")

    with pytest.raises(subject.ProductionMarketEvidenceV2Error, match="credentials"):
    ...
    with pytest.raises(subject.ProductionMarketEvidenceV2Error, match="proxy"):
        subject.initialize_capture(
            request_path=request_path,
            durable_root=durable_root,
            collector_commit="1" * 40,
...
def test_okx_market_and_instrument_normalization_is_source_separated() -> None:
    ...
    contracts, instruments = subject.normalize_okx_instruments(
        _instrument_payload(), available_at_ms=available_at_ms
    )
...
def test_okx_ticker_staleness_and_conflicting_instruments_fail_closed() -> None:
    ...
    with pytest.raises(subject.ProductionMarketEvidenceV2Error, match="stale"):
    ...
    with pytest.raises(subject.ProductionMarketEvidenceV2Error, match="conflicting"):
        subject.normalize_okx_instruments(payload, available_at_ms=available_at_ms)
...
def test_okx_candles_require_confirmed_rows_and_exact_coverage() -> None:
    ...
    with pytest.raises(subject.ProductionMarketEvidenceV2Error, match="uncompleted"):
    ...
    with pytest.raises(subject.ProductionMarketEvidenceV2Error, match="incomplete"):
...
def test_collect_due_sample_persists_one_verified_okx_sample(tmp_path: Path) -> None:
    durable_root = tmp_path / "durable"
    request_path = tmp_path / "request.json"
