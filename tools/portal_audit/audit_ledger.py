...
def load_ledger(path: Path = LEDGER_PATH) -> dict[str, Any]:
    ...
    if (
        index.get("schema_version") != "portal-completeness-ledger-v2"
        or index.get("mode") != "living_exact_head_gate"
    ):
    ...
    classifications = {
        ...
        "expected_absent_backend_routes": _rules(
            routes.get("expected_absent"), "expected_absent_backend_routes"
        ),
        "navigation": _rows(
            section["navigation"],
            (
        ...
        "runtime_fixture_boundaries": _rows(
            runtime.get("runtime_fixture_boundaries"),
            ("path", "classification", "proves", "does_not_prove"),
            "runtime_fixture_boundaries",
        ),
        "deployment_boundaries": _rows(
            runtime.get("deployment_boundaries"),
            ("area", "status", "reason"),
    ...
    ledger = {
        ...
        "_source_sha256": canonical_digest({"index": index, "sections": section}),
    }
...
def ledger_metadata(ledger: Mapping[str, Any], head: str) -> dict[str, str]:
    return {
        ...
        "ledger_sha256": str(ledger.get("_source_sha256") or canonical_digest(ledger)),
    }
...
def issue_numbers(value: Any) -> set[int]:
    ...
    return (
        {int(match.group("number")) for match in ISSUE_RE.finditer(value)}
...
def validate_ledger(ledger: Mapping[str, Any], completed: set[int] | None = None) -> None:
    ...
    for key in ("backend_modules", "backend_routes", "frontend_pages", "bff_handlers"):
        fingerprint = inventory.get(key)
        if (
            not isinstance(fingerprint, Mapping)
            or not isinstance(fingerprint.get("count"), int)
            or not DIGEST_RE.fullmatch(str(fingerprint.get("sha256", "")))
        ):
    ...
    entries = [
        ...
        for key, entry in classifications[section].items()
    ]
    entries += [
        (f"deployment:{index}", row)
    ...
    for key, entry in entries:
        ...
        if key.startswith("navigation:"):
            ...
            seen.add(pair)
            if entry.get("overall") == "COMPLETE" and any(
                entry.get(layer) != "COMPLETE"
                for layer in (
                    "frontend",
                    "api_boundary",
                    "backend",
    ...
    completed_issues = completed_programme_issues() if completed is None else completed
    stale = sorted(issue_numbers(ledger) & completed_issues)
    if stale:
        raise AuditLedgerError(
            "ledger references programme Issues marked COMPLETE: "
...
def validate_inventory(data: Mapping[str, Any], ledger: Mapping[str, Any]) -> None:
    ...
    for key, value in actual.items():
        ...
        if len(value) != expected["count"] or digest != expected["sha256"]:
            raise AuditLedgerError(
                f"{key} drift requires an explicit ledger update; "
                f"expected {expected['count']}/{expected['sha256']}, "
                f"got {len(value)}/{digest}"
            )

    expected_digest = ledger["inventory"]["composition_signature_sha256"]
    ...
    routes = {item["route"] for item in actual["backend_routes"]}
    for prefix in classifications["expected_absent_backend_routes"]:
